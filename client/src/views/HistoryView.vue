...
<template>
  <div class="max-w-2xl mx-auto p-10 pt-20">
    ...
    <div v-if="!loading && auth.user.logged_in && records.length === 0" class="text-center text-gray-400 dark:text-gray-500 py-20">
      ...
      <div v-for="r in records" :key="r.id"
        ...
        <div class="flex items-center justify-between px-5 py-3 cursor-pointer" @click="toggleExpand(r.id)" data-umami-event="history-item-toggle">
          <div class="flex items-center gap-3">
            <span class="text-lg font-bold text-blue-700 dark:text-blue-400">「{{ r.word }}」</span>
            <span class="text-xs text-gray-400 dark:text-gray-500 bg-gray-100 dark:bg-gray-800 px-2 py-0.5 rounded">
              {{ r.mode === 'deep' ? '深度' : '快速' }}
            </span>
          </div>
          <div class="flex items-center gap-3">
            <span class="text-xs text-gray-400 dark:text-gray-500">{{ formatTime(r.created_at) }}</span>
            <span class="text-gray-300 dark:text-gray-600 text-sm">{{ selectedId === r.id ? '收起' : '展开' }}</span>
          </div>
        </div>
        <div v-if="selectedId === r.id" class="px-5 pb-4 border-t border-gray-100 dark:border-gray-800 pt-3 space-y-3">
          <div v-if="r.quick_answer" class="p-3 bg-blue-50 dark:bg-blue-950/30 rounded-lg">
            <h4 class="text-xs font-bold text-blue-700 dark:text-blue-400 mb-1">⚡ 快速回答</h4>
            <p class="text-sm whitespace-pre-wrap">{{ r.quick_answer }}</p>
          </div>
          <div v-if="r.dict_result" class="p-3 bg-emerald-50 dark:bg-emerald-950/30 rounded-lg">
            <h4 class="text-xs font-bold text-emerald-700 dark:text-emerald-400 mb-1">📖 汉典释义</h4>
            <DictDisplay :dict-result="r.dict_result" />
          </div>
          <div v-if="r.deep_think" class="p-3 bg-purple-50 dark:bg-purple-950/30 rounded-lg">
            <h4 class="text-xs font-bold text-purple-700 dark:text-purple-400 mb-1">🧠 深度分析</h4>
