    ...
    <header
      ...
      <div class="hidden lg:flex items-center gap-4 text-sm">
        <nav class="flex gap-4 mr-2">
          ...
          <RouterLink to="/history" class="text-gray-500 hover:text-gray-800" data-umami-event="header-page-switch-history">历史记录</RouterLink>
          <RouterLink to="/profile" class="text-gray-500 hover:text-gray-800" data-umami-event="header-page-switch-account">个人</RouterLink>
        </nav>
        <span v-if="auth.user.logged_in" class="text-gray-500">{{ auth.user.user_id }}</span>
        <LoginButtons v-if="!auth.user.logged_in" size="sm" />
        <button v-if="auth.user.logged_in" @click="auth.logout" class="text-gray-400 hover:text-red-500" data-umami-event="header-logout">退出</button>
      </div>
      <button @click="mobileMenuOpen = !mobileMenuOpen"
        class="lg:hidden p-2 text-gray-600 hover:text-gray-800 -mr-2"  data-umami-event="header-mobile-menu-open">
    ...
    <div v-if="mobileMenuOpen"
      ...
      <div class="px-6 py-4">
        <nav class="flex flex-col gap-1 text-sm">
          ...
          <RouterLink to="/history" class="text-gray-600 hover:text-gray-800 py-1.5" @click="mobileMenuOpen = false" data-umami-event="header-page-switch-history">历史记录</RouterLink>
          <RouterLink to="/profile" class="text-gray-600 hover:text-gray-800 py-1.5" @click="mobileMenuOpen = false" data-umami-event="header-page-switch-account">个人</RouterLink>
        </nav>
        ...
        <div v-else class="flex items-center gap-3 text-sm pt-3 border-t border-gray-100 mt-2">
          ...
          <button @click="auth.logout" class="text-gray-400 hover:text-red-500" data-umami-event="header-logout">退出</button>
        </div>
      </div>
    </div>
