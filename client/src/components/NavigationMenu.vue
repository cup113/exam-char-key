...
<template>
    <NavigationMenuRoot class="fixed bottom-0 w-full p-2 bg-secondary-400 md:right-4 md:bottom-4 md:w-auto md:rounded-xl">
        <NavigationMenuList class="flex md:flex-col items-center justify-center gap-4">
            <NavigationMenuItem v-for="link in links" :key="link.to"
                class="rounded-lg text-primary-700 hover:text-accent-700 px-2 py-1 font-bold">
                <RouterLink :to="link.to" class="rounded-xl flex items-center flex-col">
                    <Component :is="link.icon" class="w-10 h-10"></Component>
                    <div>{{ link.name }}</div>
                </RouterLink>
            </NavigationMenuItem>
        </NavigationMenuList>
