...
const processedKeyword = computed(() => {
    return stress_keyword(props.note.get_keyword(), queryStore.lastQuery.word);
});
...
const processedDetail = computed(() => {
    return stress_keyword(props.note.core_detail, queryStore.lastQuery.word);
});
...
<template>
    <div class="rounded-lg shadow px-4 py-2">
        <div class="font-bold text-center">{{ note.name_passage }}</div>
