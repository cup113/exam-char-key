...
<template>
    <div class="bg-white rounded-lg shadow-md p-6 mb-6">
        ...
        <div class="space-y-4">
            <div>
                <textarea v-model="newPrompt"
                    ...
                    rows="3" placeholder="粘贴模卷文言文（一）完整内容（包含答案），AI 提取题目，可用于制作数据集" required></textarea>
            </div>
        ...
        <!-- 任务列表 -->
        <div v-if="tasks.length > 0" class="mt-6 grid grid-cols-1 gap-6">
            <div v-for="task in tasks" :key="task.id"
                class="bg-white rounded-lg shadow-md overflow-hidden border border-secondary-200">
                <div class="p-4 border-b border-secondary-200 bg-secondary-50">
                    <div class="flex justify-between items-center">
                        <h3 class="font-semibold text-sm text-secondary-700">任务 {{ task.id }}</h3>
                        <button @click="removeTask(task.id)" class="text-danger-500 hover:text-danger-700 text-sm">
                            删除
                        </button>
                    </div>
                </div>
                <div class="p-4">
                    <div class="h-64 overflow-y-auto border border-secondary-200 rounded-md p-2 bg-secondary-50">
